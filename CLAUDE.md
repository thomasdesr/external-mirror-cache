...
7. On any other 200 OK: streams response to S3 cache, then redirects client. If the cache write fails with a stale copy present, the fallback policy decides between serving stale and erroring
...
- `fallback.go` - `FallbackPolicy`: controls when stale cached content is served on upstream errors or cache-write failures
